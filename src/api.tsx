...
export const usePostQuickTest = (
  quickTest: IQuickTest | undefined,
  processId: string,
  onSuccess?: () => void,
  onError?: (error: any) => void
) => {
...
export const useGetUuid = (
  ...
  // on new hash
  React.useEffect(() => {
    if (!uuidHash || uuidHash === '' || !isNew) return;
...
export const useGetTests = (onError?: (error: any) => void) => {
  ...
  React.useEffect(() => {
    api
      .get('/api/antigentests', { headers: header })
      .then((response) => {
        setResult(response.data);
      })
      .catch((error) => {
...
export const useStatistics = (
  ...
  React.useEffect(() => {
    if (!statisticData) {
      api
        .get('/api/quickteststatistics', { headers: header })
        .then((response) => {
...
export const useGetKeycloakConfig = (
  ...
  React.useEffect(() => {
    api
      .get('/api/config/keycloak.json', { headers: header })
      .then((response) => {
        setResult({
...
export const useGetContextConfig = (
  ...
  React.useEffect(() => {
    api
      .get('/api/config/context.json', { headers: header })
      .then((response) => {
        setResult(response.data);
        if (onSuccess) {
          onSuccess(response?.status);
...
export const useGetPositiveForTimeRange = (
...
export const useGetPDF = (
  ...
  React.useEffect(() => {
    if (hash) {
      ...
      api
        .get(uri, { headers: header, responseType: 'arraybuffer' })
        .then((response) => {
...
export const useGetUsers = (
  ...
  const createUser = (user: IUser) => {
    return api.post(baseUri, JSON.stringify(user), { headers: header });
  };

  const readUser = (user: IUser) => {
    const uri = baseUri + '/' + user.id;
    return api.get(uri, { headers: header });
  };
...
export const useGetGroups = (
  ...
  const createGroup = (group: IGroupDetails) => {
    return api.post(baseUri, JSON.stringify(removeEmpty(group)), {
      headers: header,
    });
  };
  ...
  const updateGroup = (group: IGroupDetails) => {
    ...
    return api.put(uri, JSON.stringify(removeEmpty(group)), {
      headers: header,
    });
  };
  ...
  const removeEmpty = (group: IGroupDetails): IGroupDetails => {
    if (group) {
      // group.bsnr = group.bsnr || undefined;
...
export const addGroupAsChild = (
  ...
  return api.post(uri, JSON.stringify({ groupId: childGroupId }), {
    headers: header,
  });
};
