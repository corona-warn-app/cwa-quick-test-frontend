...
const UserTable = (props: any) => {
  ...
  const [
    ...
    updateUser,
    deleteUser,
  ] = useGetUsers(handleSuccess, props.handleError);
  ...
  const removeUsers = (user: IDisplayUser | IUser) => {
    users.splice(
      users.findIndex((_user) => user.id === _user.id),
      1
    );
    setUsers(users);
  };
  ...
  const userUpdate = (user: IUser) => {
    if (editUser && editUser.username) {
      ...
      updateUser(user).then(() => {
        if (fuser && fuser.subGroup !== user.subGroup && keycloak.token && user.subGroup) {
            ...
            >
              ...
              <tbody>
                {users.map((u, i) => (
                  <tr key={i}>
                    <td>
                      {u.subGroup || (ownUserId && u.id === ownUserId) ? (
                        <></>
                      ) : (
                        <OverlayTrigger
                          placement='top-end'
