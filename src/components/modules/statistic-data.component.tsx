...
const StatisticDataRow = (props: any) => {
  ...
  return (
    <Table
      ...
      hover
      responsive
      className='width-eight-houndred'
    >
      <thead>
        <tr>
          <th>&nbsp;</th>
      ...
      <tbody>
        {statisticRows.map((statiscData: DisplayStatisticData, i: number) => (
          <tr key={statiscData.key}>
            <td>
              <strong>{statiscData.label}</strong>
            </td>
            <td>{statiscData.ratTestCount}</td>
            <td>
              {statiscData.ratTestCount > 0
                ? statiscData.ratPositiveTestCount +
                  ' ( ' +
                  ((100 * statiscData.ratPositiveTestCount) / statiscData.ratTestCount).toFixed(2) +
                  '% )'
                : 0}
            </td>
            {!pcrEnabled ? (
              ...
              <>
                ...
                <td>
                  {statiscData.pcrTestCount > 0
                    ? statiscData.pcrPositiveTestCount +
                      ' ( ' +
                      ((100 * statiscData.pcrPositiveTestCount) / statiscData.pcrTestCount).toFixed(
                        2
                      ) +
                      '% )'
                    : 0}
            ...
            <td className='width-one'>
              <Button
                className='btn-icon delete-icon mx-1'
                onClick={() => {
                  props.deleteRow(statiscData.key);
                }}
              />
            </td>
          </tr>
    ...
    <Row className='add-statistic'>
      ...
      >
        <Form.Label className='input-label'>
          <strong>{t('translation:addStatisticRow')}</strong>
        </Form.Label>
      </Col>
      ...
      <Col
        ...
        className='pr-0 pl-2'
      >
        <Form.Label className='input-label mr-2'>{t('translation:timerange')}</Form.Label>
      </Col>
      <Col
        xs='12'
        md='7'
        ...
        <Row>
          <Col className='add-statistic-item'>
            ...
            <Button
              ...
              variant='light'
              onClick={() => {
                props.addRow(dateValidFrom, dateValidTo);
              }}
