...
const PagedList = (props: any) => {
  ...
  React.useEffect(() => {
    ...
    if (curPage && curPage > 0 && curPage <= _pages && dataToFilter) {
      ...
      const endIndex = curPage === _pages ? dataToFilter.length : curPage * displayItemCount;

      setDataToShow(dataToFilter.slice(startIndex, endIndex));

      // pagination
      const p: JSX.Element[] = [];
      let index = curPage === 1 ? curPage : curPage - 1;

      let max = curPage + 1 < _pages ? curPage + 1 : curPage;
      ...
      if (useBeginElipsis) {
        p.push(<Pagination.Ellipsis key='eb' />);
      }
      ...
      if (useEndElipsis) {
        p.push(<Pagination.Ellipsis key='ee' />);
      }
