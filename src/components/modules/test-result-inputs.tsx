...
const TestResultInputs = (props: any) => {
  ...
  React.useEffect(() => {
    if (testManufacturerId) {
      const desc = getValueSetDisplay(
        testManufacturerId,
        context!.valueSets![Value_Sets.TestManufacturer]
      );
      if (desc) {
      ...
      <Container className='form-flex p-0 '>
        {testType === TestType.RAT ? (
          <>
            {/* test-ID input */}
            <FormGroupInput
              controlId='formTestIdInput'
              title={t('translation:test-id')}
              value={testId}
              ...
              datalist={
                tests
                  ? tests.map((i: ITests) => (
                      <option
                        key={i.testBrandId}
                        value={i.testBrandId + ' - ' + i.testBrandName}
                      />
                    ))
                  : undefined
              }
            ...
            {/* test-name input */}
            <FormGroupInput
              controlId='formTestNameInput'
              title={t('translation:test-name')}
              value={testName}
            ...
            <FormGroupValueSetSelect
              controlId='formTestManufactorersInput'
              title={t('translation:testManufacturers')}
              infoText={
                <>
                  {t('translation:RAT-list-info')}{' '}
                  <a
                    className='rat-list-info-link'
                    href={t('translation:RAT-list-info-link')}
                    target='blank'
                  >
                    {t('translation:RAT-list-info-link-text')}
                  </a>
                </>
              }
          ...
          <FormGroupInput
            controlId='formpcrTestNameInput'
            title={t('translation:pcrTestManufacturers')}
            value={pcrTestName}
            required
            maxLength={80}
        ...
        <FormGroupRadio
          ...
          onChange={() => {
            testType === TestType.RAT
              ...
              : setTestResult(TestResult.PCR_POSITIVE);
          }}
          name='result-radios'
          required
        />
        ...
        <FormGroupRadio
          ...
          onChange={() => {
            testType === TestType.RAT
              ...
              : setTestResult(TestResult.PCR_NEGATIVE);
          }}
          name='result-radios'
          required
        />
        ...
        <FormGroupRadio
          ...
          onChange={() => {
            testType === TestType.RAT
              ...
              : setTestResult(TestResult.PCR_INVALID);
          }}
          name='result-radios'
          required
        />
